...
def augment(episode_path: str, pairs: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    for eps_id, eps in enumerate(episodes):
        ...
        xpq_pairs = pair_embed_ss.search(embeds)
...
if __name__ == '__main__':
    el = EmbeddingLoader('emb')
    pairs_path = ''

    pairs = np.load(pairs_path)
    pairs_x = pairs[:, 0]
    embeds = el.load(pairs_x)

    pair_embed_ss = VectorSimilaritySearch(embeds, num_candidate_neighbors=16, num_actual_neighbors=10, use_gpu=True)
